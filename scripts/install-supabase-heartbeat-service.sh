...
ON_UNIT_ACTIVE_SEC="${HOME_SERVER_SUPABASE_HEARTBEAT_INTERVAL:-24h}"
...
log "Installing Supabase heartbeat systemd units..."
mkdir -p "${STATE_DIR}"
chmod 755 "${STATE_DIR}"
...
log "Running one Supabase heartbeat now..."
if ! systemctl start "${SERVICE_NAME}"; then
  warn "Initial Supabase heartbeat failed. Inspect with: journalctl -u ${SERVICE_NAME} -o cat -n 200"
fi

systemctl status "${TIMER_NAME}" --no-pager
log "Supabase heartbeat installed."
